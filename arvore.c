...
void update(No *no, char elemento, char novo_valor) {
	...
	if (no != NULL) {
		...
		if (no->valor == elemento) {
			no->valor = novo_valor;
			printf("Valor do nó atualizado na árvore.");
			return;
		} else {
			update(no->filho1, elemento, novo_valor);
			update(no->filho2, elemento, novo_valor);
			update(no->filho3, elemento, novo_valor);
...
int contar_nos(No *no) {

	if (no != NULL) {
		return 1 + contar_nos(no->filho1) + contar_nos(no->filho2) + contar_nos(no->filho3);
	} else {
		return 0;
	}
}
